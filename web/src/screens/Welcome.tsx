...
export function Welcome({ onNavigate }: Props) {
  ...
  if (mode === 'create') return <CreateForm onBack={() => setMode('pick')} onDone={() => onNavigate('dashboard')} />;
  if (mode === 'join') return <JoinForm onBack={() => setMode('pick')} onDone={() => onNavigate('dashboard')} />;
...
function CreateForm({ onBack, onDone }: { onBack: () => void; onDone: () => void }) {
  ...
  const handleCreate = async () => {
    ...
    // In production this calls create_room RPC. For now we simulate.
    setCode('K7P4Q2');
  };
  ...
  if (code) {
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '60px 24px', textAlign: 'center', background: theme.bg, minHeight: '100dvh' }}>
        ...
        <div style={{ fontSize: 36, fontWeight: 800, letterSpacing: 4, color: theme.accent, background: theme.accentBg, borderRadius: 12, padding: '20px 0', marginBottom: 24 }}>{code}</div>
        <button onClick={onDone} style={btn}>Go to your room</button>
      </div>
    );
  }
...
function JoinForm({ onBack, onDone }: { onBack: () => void; onDone: () => void }) {
  ...
  const handleJoin = async () => {
    ...
    // In production this calls join_room RPC.
    onDone();
  };
